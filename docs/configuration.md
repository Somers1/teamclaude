...
| `warmupSeconds` | [Keep-warm](quota.md#keep-warm) interval in seconds (`0` = off, the default; CLI `warmup`). Spawns a minimal `claude` to start 5h timers — **spends a little quota**, unlike the probe |
| `staggerWarmup` | Warm one idle account per interval, most flexible first (`false` = warm every idle account each interval; CLI `warmup stagger <seconds>`) |
...
| `distributeSessions` | Spread concurrent Claude Code sessions across equal-priority accounts, each session pinned to one account for cache reuse (`true` by default; `false` = quota-driven rotation only). Session tracking and readout is always on regardless — see [Session-aware routing](routing.md#session-aware-routing) |
| `rescueHours` | An account whose governing weekly bucket resets within this many hours jumps to the front of the selection queue — its remaining headroom is wiped at the reset, so it is spent first (default `24`) — see [Choosing an account](routing.md#choosing-an-account) |
